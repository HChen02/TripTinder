...
def recommend_via_similarity(df: pd.DataFrame, keys: list[str], group_preferences: list[list[str]], top_k: int = 5) -> pd.DataFrame:
    ...
    return df.sort_values(by="similarity_loss").head(top_k)[['city', 'similarity_loss', 'flight_price'] + keys]

# Example usage
if __name__ == "__main__":
    filepath = "merged_onlyEurope.csv"
    df_valid = pd.read_csv(filepath)

    # Example group input
    group_origins = ["LIS", "BLA", "BUD"]  # Each user's origin airport code
    ...
    maximum_group_budgets = [1500, 1500, 1500]
    
    # Filter and recommend

    #Filter by desired flight prices
